...
CURRENT_DESCRIPTION = "İlk resmi sürüm! Yeni arayüz, gelişmiş tweak motoru ve otomatik güncelleme sistemi eklendi."
GITHUB_API_URL = "https://api.github.com/repos/CDB1646/multi-optimizer/releases/latest"
VERSION_FILE = "Version.json"
UI_DIR = os.path.join(os.path.dirname(__file__), "UI")

def ensure_version_file():
    """Version.json dosyası yoksa varsayılan bilgilerle oluşturur."""
    if not os.path.exists(VERSION_FILE):
        try:
            ...
            logging.info(f"Version.json oluşturuldu: {CURRENT_VERSION}")
        except Exception as e:
            logging.error(f"Version.json oluşturma hatası: {e}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        ensure_version_file() # Dosyayı kontrol et veya oluştur
    ...
    def update_finished(self, data):
        ...
        try:
            ...
            import re
            clean_notes = re.sub(r'\[([^\]]+)\]\([^\)]+\)', r'\1', raw_notes)
            clean_notes = clean_notes.replace('**', '').replace('__', '').replace('#', '')
            clean_notes = re.sub('<[^<]+?>', '', clean_notes).strip()
            
            # Version.json dosyasını güncelle
            update_info = {
                "name": data.get("name", "multi-optimizer").strip(),
            ...
            logging.info("Version.json GitHub verileriyle güncellendi.")
            ...
            if latest_version != current_version:
                ...
                msg = QMessageBox(self)
                msg.setWindowIcon(QIcon(get_ui_path("app_icon.ico")))
                msg.setWindowTitle("Sistem Güncel")
                msg.setText(f"En son sürümü kullanıyorsunuz: <b>{current_version}</b>")
                msg.setInformativeText(f"<b>Bu sürümdeki yenilikler:</b>\n\n{clean_notes}")
